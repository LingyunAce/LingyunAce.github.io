...
function validateConfig(registry, config) {
  ...
  const checkInstances = (slot, instances) => {
    if (!Array.isArray(instances)) {
      throw new Error(`components.yml 插槽 "${slot}" 的值必须是数组`)
    }
    for (const instance of instances) {
      if (!instance || typeof instance !== 'object' || typeof instance.component !== 'string') {
        throw new Error(`components.yml 插槽 "${slot}" 存在格式错误的组件实例`)
      }
      const id = instance.component
      if (!registry.has(id)) {
